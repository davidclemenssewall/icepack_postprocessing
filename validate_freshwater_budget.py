...
trcrn_dict = {17: 'alvln',
              ...
              20: 'hpndn_t',
              21: 'ipndn_t'}
run_dict = {"test_freshwater_budget": None,
            "test_freshwater_budget_nodynamics": None,
            "test_freshwater_budget_nodynamics_noitd": None,
            "test_freshwater_budget_nodynamics_rfmin1": None,
            "base": None,
            "test_pond_options_nodynamics": None,
            "flush_pond_apnd_issue_nodynamics": None,
...
ds = hist_dict['test_freshwater_budget']
...
if True:
    comp = "test_pond_options_fixedhypsfrbd_nodynamics"#
    # Check whether or not dataarrays are identical
    for ni in [1, 2, 3, 4]:
        print(ni)
        for key, da in hist_dict["aicen_hocn_issues_nodynamics"].sel(ni=ni).data_vars.items():
...
# compute pond budget
keys = ['test_freshwater_budget',
        'test_freshwater_budget_nodynamics',
        #'test_freshwater_budget_nodynamics_noitd',
        #'test_freshwater_budget_nodynamics_rfmin1',
        'test_pond_options_nodynamics',
        "flush_pond_apnd_issue_nodynamics",
...
for i in [0, 1, 2]:
    ni = i + 1
    plot_freshwater_budget(hist_dict['test_freshwater_budget'], ni, axs[i])
...
# Plot pond budget no dynamics
f, axs = plt.subplots(3, 1, sharex=True, figsize=(10, 10))

for i in [0, 1, 2]:
    ni = i + 1
    plot_freshwater_budget(hist_dict['test_freshwater_budget_nodynamics'], ni, axs[i])
    axs[i].legend(loc='lower left')
    axs[i].set_ylabel("meltwater (m), ni:" + str(ni))
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                    datetime.datetime.fromisoformat('2015-08-15')]
axs[-1].set_xlim(xlim)
f.suptitle("Icepack No Dynamics")
plt.show()

# Plot pond budget no dynamics or itd
f, axs = plt.subplots(3, 1, sharex=True, figsize=(10, 10))

for i in [0, 1, 2]:
    ni = i + 1
    plot_freshwater_budget(hist_dict['test_freshwater_budget_nodynamics_noitd'], ni, axs[i])
    axs[i].legend(loc='lower left')
    axs[i].set_ylabel("meltwater (m), ni:" + str(ni))
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                    datetime.datetime.fromisoformat('2015-08-15')]
axs[-1].set_xlim(xlim)
f.suptitle("Icepack No Dynamics No ITD")
plt.show()

# Plot pond budget no dynamics and rfmin=1
f, axs = plt.subplots(3, 1, sharex=True, figsize=(10, 10))

for i in [0, 1, 2]:
    ni = i + 1
    plot_freshwater_budget(hist_dict['test_freshwater_budget_nodynamics_rfmin1'], ni, axs[i])
    axs[i].legend(loc='lower left')
    axs[i].set_ylabel("meltwater (m), ni:" + str(ni))
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                    datetime.datetime.fromisoformat('2015-08-15')]
axs[-1].set_xlim(xlim)
f.suptitle("Icepack No Dynamics rfracmin=1")
plt.show()

# Plot pond budget fixed hyps with no dynamics
f, axs = plt.subplots(3, 1, sharex=True, figsize=(10, 10))

for i in [0, 1, 2]:
    ni = i + 1
    plot_freshwater_budget(hist_dict['test_pond_options_fixedhyps_nodynamics'], ni, axs[i])
    axs[i].legend(loc='lower left')
    axs[i].set_ylabel("meltwater (m), ni:" + str(ni))
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                    datetime.datetime.fromisoformat('2015-08-15')]
axs[-1].set_xlim(xlim)
f.suptitle("Icepack Fixedhyps No Dynamics")
plt.show()

# Simplest demonstration of problem, residual 

# Plot residual of pond water
run_plot_dict = {#"test_freshwater_budget": [1,2,3],
                 "test_freshwater_budget_nodynamics": [1,2,3],#,2,3],
                 "test_pond_options_nodynamics": [1,2,3],
                 "aicen_issue_nodynamics": [1,2,3],
                }
var_names = ['frshwtr_residual', 'volp', 'apndn', 'hpndn', 'aice', 'vice','rdpnd', 
             'aicen',
             'expndn', 'flpndn', 'frpndn', 'rfpndn', 'apnd', 'hpnd','alvl',
             'melts', 'meltt', 'rfpnd', 'flpnd', 'expnd', 'frpnd', 'mipnd']
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
...
# explore fixedhyps
run_plot_dict = {"test_pond_options_fixedhyps_nodynamics": [1,2,3],
                 }
var_names = ['frshwtr_residual', 'volp', 'apndn', 'hpndn', 'aice', 'vice','rdpnd', 
             'aicen',
             'expndn', 'flpndn', 'frpndn', 'rfpndn', 'apnd', 'hpnd','alvl', 'alvln',
             'melts', 'meltt', 'rfpnd', 'flpnd', 'expnd', 'frpnd', 'mipnd']
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                  datetime.datetime.fromisoformat('2015-09-01')]
...
# explore fixedhyps, freeboard category
run_plot_dict = {"test_pond_options_fixedhyps_nodynamics": [1,2,3],
                 "test_pond_options_fixedhypsfrbd_nodynamics": [1,2,3],
                 }
var_names = ['frshwtr_residual', 'volp', 'apndn', 'hpndn', 'aice', 'vice','rdpnd', 
             'aicen',
             'expndn', 'flpndn', 'frpndn', 'rfpndn', 'apnd', 'hpnd','alvl', 'alvln',
...
# Plot residual of pond water
run_plot_dict = {#"test_freshwater_budget": [1,2,3],
                 "test_freshwater_budget_nodynamics": [1]#,2,3],
                }
var_names = ['liq_diff', 'liq_in', 'liq_out']
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                  datetime.datetime.fromisoformat('2015-09-01')]
...
# Plot residual of pond water no itd
run_plot_dict = {
                 "test_freshwater_budget_nodynamics_noitd": [1,2,3],
                }
...
# Plot residual of pond water no itd, cumulative
run_plot_dict = {
                 "test_freshwater_budget_nodynamics_noitd": [1,2,3],
                }
var_names = ['liq_diff',
             'expnd', 'flpnd', 'frpnd', 'rfpnd',
             'melts', 'meltt']
xlim = [datetime.datetime.fromisoformat('2015-06-01'),
                  datetime.datetime.fromisoformat('2015-08-15')]
